...
//
// The kernel and cellspec carry the issue as opaque bytes on
// `contract.task` — neither learns what a CDS issue is. This package is where
// the meaning lives, and it is the ONLY place it lives: both CDS seats admit
// the same bytes through Admit and render them through Render, so α and β are
// told the same thing by construction rather than by two formatters agreeing.
//
...
func Admit(raw []byte) (Issue, error) {
	if len(raw) == 0 {
		return Issue{}, fmt.Errorf("cds issue: contract carries no task")
	}
