...
// AC4, and the defect this whole change exists for. A patch's hunks show only
...
// AC5: every degraded path is reported as ITS OWN fact.
func TestReconstructDegradedPathsAreDistinct(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"matter does not apply": {repo, head, stale, "does not apply"},
