...
| 4 | **γ artifact consistency (protocol-supplied context)** | When the invoking protocol supplies a γ-artifact-of-record (e.g. `.cdd/unreleased/{N}/gamma-scaffold.md` on `origin/cycle/{N}`), verify it is present and consistent with the contract per `review/SKILL.md` rule 3.11b. Its absence is a finding only if the contract under review explicitly names it as a deliverable — then it is an ordinary AC-coverage finding at the demonstrated severity. The former hard gate (auto-RC, D-severity, `protocol-compliance`) is retired; γ-side pre-dispatch enforcement (`gamma/SKILL.md` §Pre-dispatch γ scaffold check) is the structural guarantee that the scaffold exists under the cycle protocol. | Historically this row hard-gated scaffold presence to prevent protocol bypass where δ dispatches α→β directly without γ coordination (per `cnos.cds/skills/cds/CDS.md` §"Field 6: Actor collapse rule" and §"Development lifecycle" → §"State machine"). That standing gate is retired so review is executable against any diff + contract; scaffold discipline is now enforced at γ dispatch time, and β reads protocol-supplied artifacts as review context. |
| 5 | **Close-keyword presence** | Before push, verify the planned merge commit subject carries a GitHub close-keyword for the cycle's parent issue — the subject must contain `Closes`, `Fixes`, or `Resolves` immediately followed by `#{N}`, case-insensitive (oracle command below the table). A bare issue reference such as `(#{N})` does not trigger GitHub's auto-close and does not satisfy this row; rewrite the planned subject to include the close-keyword before merging. | Merge lands but the parent issue is left OPEN indefinitely because the subject referenced the issue number without a close-keyword. *Derives from: cnos#368 — cycle #367's merge subject `Merge cycle/367 — Design CDD contract/receipt validation surface (#367)` used a bare `(#367)` reference with no close-keyword; the issue remained OPEN ~24h until manually closed. `gamma/SKILL.md §2.10`'s close-state assertion (added the same cycle) is the second-layer catch if this row is ever skipped.* |
