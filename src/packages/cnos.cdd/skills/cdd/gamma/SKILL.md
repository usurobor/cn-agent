...
**Relation to `review/SKILL.md` rule 3.11b.** Under earlier revisions of rule 3.11b, a missing `gamma-scaffold.md` at review time fired REQUEST CHANGES (D-severity, `protocol-compliance`); that review-side gate is retired — current rule 3.11b treats protocol-supplied artifact locations as optional review context, with absence a finding only when the contract names the scaffold as a deliverable. γ-side pre-dispatch enforcement (this gate) is therefore the structural guarantee that the artifact exists, paying once *per cycle* at scaffold time instead of per RC round.

**Empirical anchor — cycle #369.** β R1 fired rule 3.11b as D1 against α's review-ready signal (missing scaffold); γ recovered via path (a) at `227d2373`; β R2 APPROVED at `4e179db6`; merged at `ff54f2a0`. All 10 ACs were met at R1 — the one "no" row was the missing scaffold. That RC fired under the since-retired revision of rule 3.11b; with the review-side gate retired, the γ-side gate stands alone as the scaffold-presence enforcement point.
...
**Protocol exemption.** γ may skip the scaffold *only if* the sub-issue body carries an explicit `## Protocol exemption` section naming the reason (emergency patch, infra-only change with operator override, wave-manifest-as-γ-artifact per `operator/SKILL.md` §10). This exemption-discoverability discipline formerly lived in `review/SKILL.md` §3.11b "Exemption discoverability" (retired); it is now γ-side law only.
...
14. if the cycle's receipt has `protocol_gap_count > 0` (≥1 finding tagged `cdd-skill-gap` / `cdd-protocol-gap` / `cdd-tooling-gap` / `cdd-metric-gap`), `.cdd/unreleased/{N}/cdd-iteration.md` exists with each finding structured per `post-release/SKILL.md` Step 5.6b, **and** `.cdd/iterations/INDEX.md` has a row for cycle N. If any finding shipped to a different repo, `.cdd/iterations/cross-repo/{target}/{slug}/` exists with bundle + `LINEAGE.md`. If `protocol_gap_count == 0`, no iteration file is required (per [`ROLES.md §4b.4`](../../../../../../docs/concepts/ROLES.md), [`epsilon/SKILL.md §1`](../epsilon/SKILL.md), and [`activation/SKILL.md §22`](../activation/SKILL.md)); the INDEX row is also not required for empty-findings cycles.
