...
See `RELEASE-LEVEL-CLASSIFICATION.md` (removed from the reader surface — in git history) for the full historical analysis.
...
- **#403** (subs #406–#412) — **CDS v0.1**. `cnos.cds` package + CDS.md six-field instantiation contract + operational sub-area overlays + extraction map + empirical anchor. Per-role overlays deferred to v1.
- **#404** (subs #415–#420) — **cnos.handoff v0.1**. Wire-format doctrine for inter-agent / inter-activation / inter-repo handoff. Five sub-skills.
- **#414 / #424 / #425** — Three design essays (`DECREASING-INCOHERENCE`, `CELL-OF-CELLS`, `BOX-AND-THE-RUNNER`). `BOX-AND-THE-RUNNER` codifies remote-runner delegation as a δ-class effect surface; `delta/SKILL.md §8` pins the 6-field receipt convention.
- **#422 / #423** — Release-hygiene + P0 build-fix: READMEs rewritten v0.1-complete; `VERSION` 3.81.0 → 3.82.0; compat-stub triggers added to fix activation-validator test.
...
- **#381** — `cn activate` no longer emits a false `→ activate skill not vendored ...; using built-in ordering` warning when the skill IS vendored. `parseReadFirstOrderBlock` now requires the `<!-- read-first-order:begin -->` / `<!-- read-first-order:end -->` markers to appear on their own line (after TrimSpace), so the SKILL.md docstring mention of the marker syntax in §4.1 prose (literal backticked markers used to describe the format) is no longer matched ahead of the real block. Rendered prompt is unchanged — the fallback ordering already matched the skill on commit — but the parser now correctly identifies the skill as the source of truth, restoring the cycle/379 "skill is source of truth, fallback is degraded" invariant in observable output. Two new regression tests lock in the docstring-mention and indented-marker cases. Shipped via α-direct override per operator: 1-AC parser bugfix, recommended fix named in #381 body, no β review.

## 3.79.0

- **#380** — `cn activate --claude` and `cn activate --codex` flags spawn the AI body interactively with the activation prompt pre-loaded. The new flags replace the cn process with `claude "$prompt"` or `codex "$prompt"` via `syscall.Exec` (TTY-clean process replacement), dropping the operator into a live REPL with the body already activated. Default no-flag stdout behavior is preserved bytes-equal to 3.78.0 by construction (Option A render-capture seam: default arm has no buffer indirection). Mutual exclusion and missing-binary errors fire pre-render, proven by a "nonexistent HUB_DIR" oracle that asserts both the pre-render diagnostic AND the absence of the renderer's own error in stderr. New files: `src/go/internal/activate/spawn.go` (`//go:build unix`), `spawn_other.go` (non-unix portability stub), `spawn_test.go` (7 tests), `cmd_activate_test.go` (9 tests). Modified: `src/go/internal/cli/cmd_activate.go`. One γ step-13a patch lands in the same release commit: `alpha/SKILL.md` §2.6 row 14 SHA-citation-after-rebase invariant clause (closes the role-skill under-specification surfaced by R1 F1).

## 3.78.0

- **#379** — `agent/activate` skill ships as the single source of truth for agent self-activation at a cnos hub (`src/packages/cnos.core/skills/agent/activate/SKILL.md`, 485 lines); `cn activate`'s Go renderer (`src/go/internal/activate/activate.go`) evolves to parse the skill's §4.1 marker-bounded load-order block at every invocation, with an in-Go fallback for pre-`cn deps restore` boundary. Two γ step-13a patches land in the same release commit: β/SKILL.md §pre-merge gate row 3 tightened to enumerate the contract-validator set (closes the application gap that produced post-merge CI red); R5-activate kata P10 + activate SKILL.md `calls:` paths corrected (restores I5 + Package verification green).
...
|---------|-----|---|---|---|--------|-------|----------------|
| 3.81.0 | B− | B+ | A | C | 1 | L6 (CI-red cap: pre-existing I4 / notify infrastructure inherited from cycle #369; not cycle-introduced; β verified non-destructive merge. 0 β findings — first 0-finding result on a 9-AC code cycle. CLP r2 at issue creation eliminated all design ambiguity.) | **Activation soul: 6→2 CA skills** (#385, merge `45dbcc47`). `activate/SKILL.md §2.1 step 2` lists cap + clp only (was: cap, clp, mca, mci, coherent, agent-ops). cap gains §4 MCA + §5 MCI + §6 Coherent output absorbing operational rules from standalone files; non-absorbed boundaries named concretely (root-finding seq stays in mca; scope/specificity + MCA/MCI table stays in mci; Coherence Modes + Anti-Patterns tables stay in coherent). mca/mci/coherent/agent-ops get `activation_status: on-demand` frontmatter. AGENTS.md deleted. activate.go:506 → `{cap,clp}`. R5-activate kata P10 updated. All 9 ACs met; 0 findings (R1 APPROVED); 1 round. Dispatch: §5.1 canonical multi-session. Hub-side downstream cleanup (cn-sigma RULES.md, spec/*) deferred per scope. |
| 3.79.0 | B− | B+ | A | C | 2 | L6 (cycle cap: L6; γ axis capped at C per `release/SKILL.md` §3.8 CI-red clause — pre-existing I4 / notify infrastructure inherited from cycle #369, not introduced or scoped to fix by #380; β verified non-destructive merge against bare main with identical failure shape) | **`cn activate --claude` / `--codex` interactive-spawn flags** (#380, merge `770ea1b4`). Two new flags replace the cn process with `claude "$prompt"` or `codex "$prompt"` via `syscall.Exec` (TTY-clean process replacement), dropping the operator into a live REPL with the body activated. Default no-flag stdout behavior preserved bytes-equal to 3.78.0 by construction (Option A render-capture seam: default arm passes `inv.Stdout` straight through; bytes-equal property structural, not test-only). New `spawn.go` (`//go:build unix`, exports `Spawn` + `CheckSpawnBinary`) + `spawn_other.go` (non-unix portability stub) + `spawn_test.go` (7 tests) + `cmd_activate_test.go` (9 tests). β R1 REQUEST CHANGES with one B-severity honest-claim finding (F1 — pre-rebase SHAs in §CDD Trace after α applied `alpha/SKILL.md` §2.6 row 14 path (a) mid-cycle to remediate a worktree-inherited identity leak); R2 APPROVED after self-coherence-only fix-round (α applied §2.3 intra-doc repetition rule, found 9 sites — 3 beyond β's enumeration — fixed in one commit). One γ step-13a patch lands in this release: `alpha/SKILL.md` §2.6 row 14 SHA-citation-after-rebase invariant clause (structural backstop at the role-skill layer). |
| 3.78.0 | B− | B+ | B+ | C | 1 | L6 (cycle cap: L6; γ axis capped at C per `release/SKILL.md` §3.8 CI-red clause) | **agent/activate skill: single source of truth + Go renderer parse** (#379, merge `a3bf7892`). New `src/packages/cnos.core/skills/agent/activate/SKILL.md` (485L) — body-agnostic six-item canonical load order (Kernel → CA skills → Persona → Operator → hub state → identity) with the layering rule cited (Soul vs Identity, cn-sigma `threads/adhoc/20260325-session2-learnings.md §3`); three-tier body-capability matrix (a) shell+git preferred (b) HTTP fetch only (c) no-fetch operator-injected; paste-testable README router template hubs adopt verbatim with only `<HUB-URL>` substitution; explicit disambiguation from `cnos.cdd/skills/cdd/activation/SKILL.md` (CDD repo activation — same word, different concern). `src/go/internal/activate/activate.go` evolved: `writePrompt` no longer carries `## Read first` ordering as in-Go literals; iterates `readFirst []readFirstItem` slice produced by `loadActivateSkillOrdering` which parses §4.1's marker-bounded numbered list via `parseReadFirstOrderBlock`; `canonicalReadFirstOrdering` fallback preserves manifest-only behavior, skill canonical on drift. `activate_test.go` adds `TestSkillIsSourceOfTruthForReadFirstOrder` (two-phase edit-and-swap, `out1 != out2` coherence assertion), `TestSkillFallback_NotVendored`, and parser unit tests; two pre-existing `TestReadFirstSection_*` tests updated to the new canonical ordering. β R1 APPROVED with zero in-cycle findings. **Post-merge CI red regressed:** I5 (SKILL.md frontmatter validation) failed because the new skill's `calls:` paths were `cnos.core/...` prefixed instead of package-skill-root-relative; Package verification (R5-activate kata P10) failed because the kata asserted the pre-#379 ordering (`persona < operator < kernel < deps < refl`) which AC3 displaced. γ step-13a in the same PRA commit: (1) β/SKILL.md §pre-merge gate row 3 tightened from "the cycle's own validator (or any CI-equivalent the cycle ships)" to an enumerated list naming `validate-skill-frontmatter.sh`, `cn-cdd-verify`, `check-version-consistency.sh`, and per-cycle katas under `src/packages/cnos.kata/katas/{N}/` with R5-activate named as the activate-surface exemplar — empirical anchor (#379 post-merge CI red) cited inline; (2) activate SKILL.md `calls:` paths corrected to `../doctrine/KERNEL.md` + `agent/{cap,clp,mca,mci,coherent,agent-ops}/SKILL.md` (validator green after patch: "66 SKILL.md validated; no findings"); (3) R5-activate kata `run.sh` P10 assertion updated to `kernel < persona < operator < {deps, refl}` (deps + refl share one hub-state line in the new ordering so relative order no longer asserted) plus `kata.md` P10 documentation update naming the canonical source-of-truth. **Cross-repo source-proposal lineage:** `usurobor/cn-sigma:.cdd/iterations/cross-repo/cnos/agent-activate-skill/` (bundle commit `1a4e25f75`; AC4 capability-matrix refinement folded in from cn-sigma branch HEAD `bdda457f5`); disposition `accepted` with delta named in issue body §Source Proposal; `landed` event addition to source-side STATUS pending operator merge of two mirror branches (`cnos:sigma/cross-repo-mirror-agent-activate-skill@212d5239`, `cn-sigma:sigma/cross-repo-status-lineage-update@89049611c`). **§9.1 triggers fired:** avoidable tooling/environmental failure (post-merge CI red on I5 + Package verification); CI red on merge commit (post-merge — triggers `release/SKILL.md §3.8` CI-red cap clause, γ axis capped at C); loaded skill failed to prevent a finding (β/SKILL.md row 3 under-specified the validator set; patched in this PRA commit). 1 review round (within target); 0 in-cycle findings; 3 post-merge findings all patched in this release commit (no findings deferred to next-MCA). **α B+** (named ACs cleanly delivered, zero in-cycle findings; 2 wiring-class regressions in peer surfaces (frontmatter format, kata assertion) reached merge — meets B+ rubric "met all ACs, ≥2 binding findings"). **β B+** (R1 APPROVED with thorough contract-integrity and per-AC verdict; missed running `validate-skill-frontmatter.sh` and R5-activate kata in pre-merge gate row 3 — application gap closed by the row 3 enumeration patch). **γ C** (CI-red cap clause; γ-scaffold failure-mode catalogue named 10 modes but did not name "β contract-validator coverage" as a pre-flag; recovery clean — all 3 mechanical fixes landed in this PRA commit per `gamma/SKILL.md §3.6`). Dispatch configuration: §5.1 canonical multi-session (one `claude -p` per role) per `gamma-scaffold.md` declaration. Hub README router adoption (cn-sigma first, cn-pi second) named as next MCA; MCI freeze in effect (5 issues at growing lag: 4 #379-deferred follow-ons + #373). |
