<!-- wave-revision: R12 -->
# Acceptance oracles — honestly classified, registry-projected (cnos#671 R12)
...
predicate (owned by no child) held in `wave_predicates:`. **R11:** that wave-boundary validator was
...
**R12: soundness-repaired** — the R11 hand-parser silently dropped CUE-valid flow-style predicate lists
(a demonstrated complete-wave false pass), so the validator now normalizes **every** input via `cue export
--out json` (owners derived from each contract's `cell.id` against the wave node ids) and is **fail-closed**
on parse loss/empty owner-predicate, with a Makefile asserting exact per-fixture exit codes (0/1/2);
...
| oracle-ownership bijection (registry ⇄ ALL contracts' mechanically-verifiable predicates; whole-wave) — R10: **wave-boundary**; R11: materialized + runs at pre-authorization; **R12: SOUNDNESS-REPAIRED** — normalizes every input via `cue export --out json` (no hand-parsed YAML), fail-closed on parse loss/empty owner-predicate (78 ⇄ 78; exit 0 iff bijective / 1 defect / 2 fail-closed; PASS bound to wave authorization) | deferred-go | **WAVE** — `.cdd/waves/cell-runtime-doctrine/wave-validators/oracle_ownership_bijection.go` |
...
| owner | predicate | classification | deferred_owner |
|---|---|---|---|
| WAVE | `wave_oracle_ownership_bijection_enforced` (whole-wave cross-contract bijection; pre-authorization; R11: materialized — `wave-validators/oracle_ownership_bijection.go`; **R12: SOUNDNESS-REPAIRED** — `cue export --out json` normalized, fail-closed, exact exit codes 0/1/2; PASS bound to wave authorization) | deferred-go | wave |
...
here** (R11) and runnable now — **R12** made it **sound** (`cue export --out json` normalized, fail-closed,
exact per-fixture exit codes). #627 S2–S3 are downstream consumers/canonicalizers, never the owner.*
